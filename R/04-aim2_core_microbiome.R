...
# 
# # Create a Venn diagram to compare healthy and CPP 
# venn_all_diseases_0.4 <- ggVennDiagram(x = CPP_list_0.4)
...
#then assign ASVs
CPP_only_ASVs_rectal <- core_members(CPP_only_rectal, detection=0, prevalence = 0.5)
CPP_endo_ASVs_rectal <- core_members(CPP_endo_rectal, detection=0, prevalence = 0.5)
CPP_healthy_ASVs_rectal <- core_members(CPP_healthy_rectal, detection=0, prevalence = 0.5)

#get the names of the genus 
CPP_endo_genera_rectal <- tax_table(CPP_RA)[CPP_endo_ASVs_rectal, "Genus"]

#try finding what genus are unique to a particular group 
#################################################
####### NOTE TO NAOMI : those ASVs could still also be in Healthy 
#################################################
Unique_to_CPP_Endo <- setdiff(CPP_endo_ASVs_rectal,CPP_only_ASVs_rectal)
Unique_to_CPP_Endo_Genera <- tax_table(CPP_RA)[Unique_to_CPP_Endo, "Genus"]

#then make a list, either with all of the diseases or just two 
CPP_list_rectal <- list(Healthy = CPP_healthy_ASVs_rectal, CPP_Only = CPP_only_ASVs_rectal, CPP_Endo = CPP_endo_ASVs_rectal)

#then make venn diagram 
venn_all_diseases <- ggVennDiagram(x = CPP_list_rectal)+ scale_fill_gradient(low ="#810f7c", high = "#8c96c6")+
...
#prune 
prune_taxa(CPP_only_ASVs_rectal,CPP_RA) %>%
  tax_table()

prune_taxa(CPP_endo_ASVs_rectal,CPP_RA) %>%
  tax_table()

prune_taxa(CPP_healthy_ASVs_rectal,CPP_RA) %>%
  tax_table()
...
#################################################
####### [TODO]: refactor vaginal to use informative variable names (see rectal)
#################################################

#try stratifying by the vaginal sample and disease instead now  
CPP_only_3 <- subset_samples(CPP_RA, `Host_disease`=="CPP")
CPP_endo_3 <- subset_samples(CPP_RA, `Host_disease`=="CPP Endo")
CPP_healthy_3 <-subset_samples(CPP_RA, `Host_disease`=="Control")

CPP_only_4 <- subset_samples(CPP_only_3,`collection_method` == "vaginal_swab")
CPP_endo_4 <- subset_samples(CPP_endo_3,`collection_method` == "vaginal_swab")
CPP_healthy_4 <-subset_samples(CPP_healthy_3, `collection_method` == "vaginal_swab")
...
venn_all_diseases_4 <- ggVennDiagram(x = CPP_list_4)+ scale_fill_gradient(low ="#810f7c", high = "#8c96c6")+
